...
static volatile bool loraRxPending = false;
static uint8_t loraRxBuf[16];
static uint16_t loraRxSize = 0;
static int16_t  loraRxRssi = 0;
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr) {
  if (size > 0 && size <= sizeof(loraRxBuf)) {
    memcpy(loraRxBuf, payload, size);
    loraRxSize = size;
    loraRxRssi = rssi;
    loraRxPending = true;
  }
...
// ─── 커맨드 처리 ───
void handleLoRaCommand(CmdPacket_t* pkt) {
  // CRC 검증
  uint8_t expectedCrc = pkt->magic ^ pkt->command ^ pkt->param;
  if (pkt->crc != expectedCrc) {
    Serial.println("[CMD] CRC fail, ignored");
    return;
  }

  if (pkt->command == CMD_RUNNING) {
    Serial.printf("[CMD] RUNNING received (RSSI:%d)\n", loraRxRssi);
    // STM32에 주행 시작 명령
    SerialSTM.println("CMD:RUN");
    // 즉시 insight 패킷 전송
    sendLoRaInsight();
  }
  else if (pkt->command == CMD_EMERGENCY) {
    Serial.printf("[CMD] EMERGENCY received (RSSI:%d)\n", loraRxRssi);
    // STM32에 비상 상승 바이너리 커맨드 전송
    sendEmergencyCommand(0x01);
    // 즉시 LoRa로 상태 보고
    sendLoRaInsight();
  }
...
void loop() {
  ...
  // ─── 1. STM32 UART 수신 ───
  // BULK_START ... TEL:... BULK_END 패킷 처리
  static bool inBulk = false;
  static uint16_t bulkCount = 0;

  while (SerialSTM.available()) {
    String incomingMsg = SerialSTM.readStringUntil('\n');
    incomingMsg.trim();

    if (incomingMsg == "BULK_START") {
      inBulk    = true;
      bulkCount = 0;
      Serial.println("[BULK] Receiving telemetry from STM32...");

    } else if (incomingMsg == "BULK_END") {
      inBulk = false;
      Serial.printf("[BULK] Received %d records → LoRa burst!\n", bulkCount);
      // LoRa 인사이트 패킷으로 요약 전송 (이미 구현된 함수 사용)
      if (sensorBuffer.totalSamples >= WINDOW_SIZE) {
        sendLoRaInsight();
      }

    } else if (inBulk && incomingMsg.startsWith("TEL:")) {
      // TEL:<ms>,<depth>,<temp>,<pitch>,<roll>,<yaw>,<volt>,<o2>,<st>,<ml>
      String p = incomingMsg.substring(4);
      // depth 파싱해서 latestDepth 업데이트
      int c1 = p.indexOf(',');
      int c2 = p.indexOf(',', c1+1);
      int c3 = p.indexOf(',', c2+1);
      if (c1 > 0 && c2 > 0) {
        latestDepth = p.substring(c1+1, c2).toFloat();
        latestTemp  = p.substring(c2+1, c3 > 0 ? c3 : p.length()).toFloat();
        if (latestDepth > sysState.maxDepthThisDive)
          sysState.maxDepthThisDive = latestDepth;
      }
      bulkCount++;
      // 10번마다 LoRa 중간 전송 (패킷 손실 방지)
      if (bulkCount % 10 == 0) {
        sendLoRaInsight();
      }
    } else {
      // 일반 메시지는 기존 파서로
      parseSTM32Data(incomingMsg);
    }
  }
  ...
  // ─── LoRa RX 커맨드 처리 (인터럽트에서 pending 세트됨) ───
  if (loraRxPending) {
    loraRxPending = false;
    if (loraRxSize == sizeof(CmdPacket_t)) {
      CmdPacket_t* pkt = (CmdPacket_t*)loraRxBuf;
      if (pkt->magic == CMD_MAGIC) {
        handleLoRaCommand(pkt);
      }
    }
  }
